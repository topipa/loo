...
loo_moment_match.default <- function(x, loo, post_draws, log_lik_i,
  ...
  loo_moment_match_i_fun <- function(i) {
    loo_moment_match_i(i, x, log_lik_i, unconstrain_pars, log_prob_upars,
                       log_lik_i_upars, max_iters, k_threshold,
                       split, cov, cores = 1, N, S, upars, orig_log_prob,
                       k = ks[i], is_method, npars, ...)
  }
